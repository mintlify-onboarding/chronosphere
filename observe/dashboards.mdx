...
---
import { CollectionIcon, ServiceIcon} from '@/components';
...
1. Click a dashboard listed in the search results to view it, or use the arrow keys
   to select the result and press <kbd>Enter</kbd> (<kbd>Return</kbd> on macOS).
...
1. In the **Dashboards** page, click the <Icon path={ mdiDotsVertical }/> three
...
To exit kiosk mode, click <kbd>Esc</kbd>.
...
1. Click **<Icon path={ mdiContentCopy } /> Copy** to copy the JSON to your clipboard,
   or click **<Icon path={ mdiDownload }/>** to download the JSON.
