...
'og:description': Learn how to create links from one telemetry type to other information in Observability Platform.
sidebarTitle: Create links
---
{/* -- dri: Adam Locke -- */}
...
click the <Icon icon="link-simple" /> link icon, and then click the link name to view the
...
```text showLineNumbers /operation/ /service/ /environment/ /severity/ /payment_gateway_service/ /authorize-card-payment/ /production/ /ERROR/
...
```text showLineNumbers /{{ service }}/ /{{ operation }}/
https://TENANT.chronosphere.io/logs/explorer?start=1h&visualization=list
...
```text showLineNumbers /{{ trace_id }}/ /{{ span_id }}/
