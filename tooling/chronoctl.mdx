...
1. Define a `chronoconfig.yaml` file to assign the converted rules to a team, and
   ...
   ```yaml copy /OWNER_TEAM/ /POLICY_TEAM/
...
1. Run the `sync prometheus` command and pass in your Prometheus rules file,
   ...
   ```shell copy /ALERTMANAGER_CONFIG/ /PROM_RULES/ /FLAGS/
...
You can now press <kbd>Tab</kbd> on the command line to automatically complete
Chronoctl commands.
