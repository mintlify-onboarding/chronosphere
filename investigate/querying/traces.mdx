...
1. Click **<Icon path={ mdiRefresh } />Run** to submit your query. You must click
   **<Icon path={ mdiRefresh } />Run** after each time you modify the query criteria.

   <Info>
You can press <kbd>Control+Return</kbd> or <kbd>Control+Enter</kbd> on your
   keyboard instead of clicking **<Icon path={ mdiRefresh } />Run** to submit a
   query.
</Info>
...
Each row is a separate span filter that Observability Platform evaluates against
   ...
   ![A screenshot of a graph of a traces search](/doc-assets/traces-graph.png)
...
1. In the **Group by** field, enter **app.payment.card_type** to additionally group
   ...
   ![Screenshot showing query results in the Span statistics tab of Trace Explorer.](/doc-assets/traces-statistics-results.png)
