...
---
import { PipelineOverviewImage} from '@/components';
...
<Columns cols={2}>
  <Card icon="wrench" title="Install - Set up Telemetry Pipeline in Linux and Kubernetes" href="/pipeline-install" />
  <Card icon="pipe" title="Create and modify pipelines - Deploy custom pipelines to collect and disseminate data" href="/pipeline-data/create-modify" />
  <Card icon="folder-tree" title="Configure - Customize Telemetry Pipeline behavior" href="/pipeline-configure" />
  <Card icon="filters" title="Processing rules - Enrich, reduce, and transform data" href="/pipeline-data/processing-rules" />
  <Card icon="earth" title="Plugins - Route telemetry data from source to destination" href="/pipeline-data/plugins" />
  <Card icon="paper-plane" title="Fleets - Create collections of distributed telemetry agents" href="/fleets" />
  <Card icon="terminal" title="Pipeline CLI - Manage pipelines through a command-line interface" href="pipeline-cli" />
</Columns>
