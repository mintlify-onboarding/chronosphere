...
1. Download the `darwin_amd64` build of `chronotf`:
   ...
   ```sh
   curl -LO https://storage.googleapis.com/chronosphere-terraform/1.12.0/chronotf_1.12.0_darwin_amd64
   ```

1. Make the `chronotf` binary executable.

   ```sh
   chmod +x chronotf_1.12.0_darwin_amd64
   ```

1. Move the binary into your `PATH`:

   ```sh
   sudo mv chronotf_1.12.0_darwin_amd64 /usr/local/bin/chronotf
   ```
...
1. Download the `darwin_arm64` build of `chronotf`:
   ...
   ```sh
   curl -LO https://storage.googleapis.com/chronosphere-terraform/1.12.0/chronotf_1.12.0_darwin_arm64
   ```

1. Make the `chronotf` binary executable:

   ```sh
   chmod +x chronotf_1.12.0_darwin_arm64
   ```

1. Move the binary into your `PATH`:

   ```sh
   sudo mv chronotf_1.12.0_darwin_arm64 /usr/local/bin/chronotf
   ```
...
1. Download the `linux_amd64` build of `chronotf`:
   ...
   ```shell
   curl -LO https://storage.googleapis.com/chronosphere-terraform/1.12.0/chronotf_1.12.0_linux_amd64
   ```

1. Make the `chronotf` binary executable:

   ```shell
   chmod +x chronotf_1.12.0_linux_amd64
   ```

1. Move the binary into your `PATH`:

   ```shell
   mv chronotf_1.12.0_linux_amd64 /usr/local/bin/chronotf
   ```
