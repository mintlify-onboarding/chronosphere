...
number of unsaved changes. An <Icon path={ mdiAlertCircle} /> alert icon indicates
...
Click the <Icon path={ mdiDotsVertical } /> three vertical dots icon and then
...
This prefix, along with optional target label names in this job, must not
    ...
    - **Service mapping**:
      ...
      - **Service label**: A label containing the service name. This label must be
        ...
        Click the Click the <Icon path={ mdiOpenInNew } /> icon to draft a
...
1. Click the <Icon path={ mdiDotsVertical } /> three vertical dots and then select
...
<Icon path={ mdiOpenInNew } /> icon to open the query in
[Metrics Explorer](/investigate/querying/metrics/explorer).
...
1. On the **Service Configuration** page, click the <Icon path={ mdiDotsVertical } />
